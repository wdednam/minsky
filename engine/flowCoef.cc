...
namespace
{
  ...
  bool parseLine(std::string const& line, std::string& pref) {
     ...
     // Construct grammar to treat floating literals separately in input formula. See https://www.boost.org/doc/libs/1_68_0/libs/spirit/doc/html/spirit/qi/reference/numeric/real.html
     ...
     //qi::rule<it, std::string()> fractional_constant = *qi::digit >> '.' >> +qi::digit | +qi::digit >> -qi::lit('.');
     //qi::rule<it, std::string()> exponent_part = (qi::lit('e') | 'E') >> -sign >> +qi::digit;
     //qi::rule<it, std::string()> floating_literal = -sign >>(( nan | inf ) >> (fractional_constant >> !exponent_part |  +qi::digit >> exponent_part) );
     
     it first = line.begin(), last = line.end();
     //return qi::phrase_parse(first, last, floating_literal ,qi::blank, pref);
     return qi::phrase_parse(first, last, -sign >> ( nan | inf ) ,qi::blank, pref);
  }
}
